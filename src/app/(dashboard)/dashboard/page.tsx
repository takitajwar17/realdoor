    ...
    <>
      ...
      <main className="mx-auto flex w-full max-w-[1520px] flex-1 flex-col gap-5 px-4 py-5 md:px-6 md:py-6">
        ...
        <section className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight md:text-3xl">Practice sessions</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Open a session, review its latest activity, or start a new one.
            </p>
          </div>
          {sessions.length > 0 ? (
        ...
        {sessions.length > 0 ? (
          <>
            <section className="grid overflow-hidden rounded-xl border border-border/80 bg-card shadow-[var(--shadow-dashboard)] sm:grid-cols-3 sm:divide-x sm:divide-border/70">
              ...
              />
              <DashboardMetric icon={MapPinIcon} label="Practice guide" value="Boston · 2026" />
            </section>
            ...
            <Card className="rounded-xl border-border/80 shadow-[var(--shadow-dashboard)]">
              ...
              <CardContent className="p-0">
                <div className="hidden grid-cols-[minmax(0,1.5fr)_minmax(180px,0.6fr)_minmax(150px,0.5fr)_auto] gap-4 border-b border-border/70 px-5 py-2.5 text-2xs font-bold uppercase tracking-[0.12em] text-muted-foreground md:grid">
                  <span>Session</span>
                  <span>Guide</span>
                ...
                <div className="divide-y divide-border/70">
                  {sessions.map((session) => (
                    <div
                      key={session.id}
                      className="grid gap-4 px-5 py-4 md:grid-cols-[minmax(0,1.5fr)_minmax(180px,0.6fr)_minmax(150px,0.5fr)_auto] md:items-center"
                    >
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <p className="font-bold">Boston LIHTC</p>
                      ...
                      <div className="text-xs">
                        <p className="font-semibold">2026 · 60% AMI</p>
